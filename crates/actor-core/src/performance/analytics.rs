...
mod tests {
    ...
    fn test_analytics_collector_creation() {
        ...
        assert!(collector.config.enable_analytics);
    }
    ...
    fn test_metric_value_creation() {
        ...
        match counter {
            MetricValue::Counter(val) => assert_eq!(val, 42),
            _ => panic!("Expected Counter"),
        }

        match gauge {
            MetricValue::Gauge(val) => assert_eq!(val, 3.14),
            _ => panic!("Expected Gauge"),
        }

        match histogram {
            MetricValue::Histogram(vals) => assert_eq!(vals.len(), 3),
    ...
    fn test_time_series_point_creation() {
        ...
        match point.value {
            MetricValue::Gauge(value) => assert_eq!(value, 42.0),
            _ => panic!("Expected Gauge variant"),
        }
        assert_eq!(point.tags.len(), 1);
        assert_eq!(point.tags.get("test"), Some(&"value".to_string()));
    }

    #[test]
    fn test_analytics_collector_metrics() {
        let config = AnalyticsConfig::default();
        let collector = AnalyticsCollector::new(config);
        ...
        let counter_value = collector.get_counter("test_counter");
        assert_eq!(counter_value, 8);
    }

    #[test]
    fn test_analytics_collector_latency() {
        let config = AnalyticsConfig::default();
        let collector = AnalyticsCollector::new(config);

        // Record some latency measurements
        collector.record_latency("test_operation", Duration::from_millis(100));
        collector.record_latency("test_operation", Duration::from_millis(200));
        collector.record_latency("test_operation", Duration::from_millis(300));
    ...
    #[test]
    fn test_analytics_collector_memory() {
        let config = AnalyticsConfig::default();
        let collector = AnalyticsCollector::new(config);

        // Record memory usage
        collector.record_memory_usage("test_component", 1024);
        collector.record_memory_usage("test_component", 2048);
    ...
    #[test]
    fn test_analytics_collector_time_series() {
        let config = AnalyticsConfig::default();
    ...
    #[test]
    fn test_analytics_collector_counters() {
        let config = AnalyticsConfig::default();
        let collector = AnalyticsCollector::new(config);
        ...
        collector.increment_counter("test", 3);

        let counter_value = collector.get_counter("test");
    ...
    #[test]
    fn test_analytics_collector_stats() {
        let config = AnalyticsConfig::default();
        let collector = AnalyticsCollector::new(config);
    ...
    #[test]
    fn test_analytics_collector_export() {
        let config = AnalyticsConfig::default();
        let collector = AnalyticsCollector::new(config);
    ...
    fn test_operation_timer() {
        let config = AnalyticsConfig::default();
        let collector = Arc::new(AnalyticsCollector::new(config));

        // Test manual finish
        {
            let timer = OperationTimer::new("test_operation".to_string(), collector.clone());
        ...
        // Test drop behavior
        {
            let _timer = OperationTimer::new("test_operation_drop".to_string(), collector.clone());
            thread::sleep(Duration::from_millis(10));
        }
    ...
    #[test]
    fn test_analytics_collector_concurrent_access() {
        let config = AnalyticsConfig::default();
        let collector = Arc::new(AnalyticsCollector::new(config));
        ...
        let counter_value = collector.get_counter("concurrent_test");
        assert_eq!(counter_value, 1000);
    }

    #[test]
    fn test_analytics_collector_disabled() {
        ...
        // Operations should still work but not record data
        collector.increment_counter("test", 1);

        let counter_value = collector.get_counter("test");
        assert_eq!(counter_value, 1); // Counter should still work even when disabled
    }

    #[test]
    fn test_analytics_collector_max_time_series_points() {
        let mut config = AnalyticsConfig::default();
        config.max_time_series_points = 5;
        let _collector = AnalyticsCollector::new(config);
