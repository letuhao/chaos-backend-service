...
use crate::interfaces::{CapLayerRegistry, CombinerRegistry, MergeRule, AcrossLayerPolicy};
use crate::enums::CapMode;
...
"#;
        ...
        let result = load_cap_layers(file.path());
        assert!(result.is_ok());
        
        let registry = result.unwrap();
        assert_eq!(registry.get_layer_order(), vec!["base"]);
        // Note: get_caps_for_layer is not part of the interface, so we can't test it directly
    }
    
    #[test]
    fn test_load_cap_layers_json() {
        let json_content = r#"
{
  "layers": [
    {
...
"#;
        ...
        let result = load_combiner(file.path());
        assert!(result.is_ok());
        
        let registry = result.unwrap();
        assert!(registry.get_rule("attack").is_some());
    }
    
    #[test]
    fn test_validation_duplicate_layer_names() {
        let yaml_content = r#"
layers:
  - name: "base"
    priority: 0
...
"#;
        ...
        assert!(matches!(result.unwrap_err(), LoaderError::ValidationError { .. }));
    }
    
    #[test]
    fn test_validation_invalid_cap_mode() {
        let yaml_content = r#"
layers:
  - name: "base"
    priority: 0
...
"#;
    ...
    #[test]
    fn test_validation_empty_layers() {
        let yaml_content = r#"
layers: []
"#;
