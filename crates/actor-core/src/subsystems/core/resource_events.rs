...
mod tests {
    ...
    #[tokio::test]
    async fn test_resource_event_manager() {
        ...
        // Check history
        let history = manager.get_event_history(None, None, Some(10)).await.unwrap();
        assert_eq!(history.len(), 1);
        assert_eq!(history[0].resource_name, "hp_current");
    }
    ...
    async fn test_event_filtering() {
        ...
        // Add a filter for high priority events only
        ...
        manager.add_event_filter(filter).await.unwrap();
        
        // Create a low priority event
        let mut event = manager.create_resource_changed_event(
            "actor1",
            "hp_current",
            100.0,
            90.0,
        ...
        // Emit the event (should be filtered out)
        manager.emit_event(event).await.unwrap();
        
        // Check history (should be empty due to filtering)
        let history = manager.get_event_history(None, None, Some(10)).await.unwrap();
    ...
    async fn test_event_listeners() {
        ...
        // Create a listener
        let listener = Arc::new(DefaultResourceEventListener::new(
        ...
        // Add the listener
        manager.add_listener(listener).await.unwrap();
