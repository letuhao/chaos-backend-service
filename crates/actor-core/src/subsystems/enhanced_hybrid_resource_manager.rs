...
mod tests {
    ...
    #[test]
    fn test_enhanced_hybrid_resource_manager_creation() {
        let rm = EnhancedHybridResourceManager::new();
        
        assert_eq!(rm.system_id(), "enhanced_hybrid_resource_manager");
        assert_eq!(rm.priority(), 100);
    }
    
    #[tokio::test]
    async fn test_shared_resource_calculation() {
        let rm = EnhancedHybridResourceManager::new();
        let actor = Actor::new("TestActor".to_string(), "Human".to_string());
        
        let shared_resources = rm.calculate_shared_resources(&actor).await.unwrap();
        
        // Check that shared resources are calculated
        assert!(shared_resources.contains_key("hp_current"));
        assert!(shared_resources.contains_key("hp_max"));
        assert!(shared_resources.contains_key("mana_current"));
        assert!(shared_resources.contains_key("mana_max"));
        assert!(shared_resources.contains_key("lifespan_years"));
    }
