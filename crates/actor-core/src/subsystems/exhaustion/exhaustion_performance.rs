...
mod tests {
    ...
    async fn test_performance_monitor() {
        ...
        monitor.record_evaluation(Duration::from_micros(500), false).await;
        monitor.record_evaluation(Duration::from_micros(2000), false).await;
    ...
    async fn test_benchmark_creation() {
        ...
        let config = BenchmarkConfig {
            name: "test".to_string(),
            actor_count: 100,
            evaluations_per_actor: 10,
            resource_ranges: HashMap::new(),
        };
        
        benchmark.add_config(config);
        assert_eq!(benchmark.test_configs.len(), 1);
    }
}
