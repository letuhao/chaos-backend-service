...
mod tests {
    ...
    #[tokio::test]
    async fn test_regeneration_manager() {
        let config = RegenerationConfig::default();
        let manager = ResourceRegenerationManager::new(config);
        
        let mut actor = Actor::new("TestActor".to_string(), "Human".to_string());
        
        // Set actor data
        let mut data = std::collections::HashMap::new();
        data.insert("hp_current".to_string(), json!(50.0));
        data.insert("hp_max".to_string(), json!(100.0));
        data.insert("vitality".to_string(), json!(15.0));
        data.insert("in_combat".to_string(), json!(false));
        data.insert("resting".to_string(), json!(true));
        actor.set_data(data);
        
        // Start HP regeneration
        manager.start_regeneration(&actor, "hp_current").await.unwrap();
        
        // Check that task was created
        let tasks = manager.active_tasks.read().await;
        assert_eq!(tasks.len(), 1);
    }
    
    #[tokio::test]
    async fn test_regeneration_conditions() {
        let config = RegenerationConfig::default();
        let manager = ResourceRegenerationManager::new(config);
        
        let mut actor = Actor::new("TestActor".to_string(), "Human".to_string());
        
        // Set actor data - in combat
        let mut data = std::collections::HashMap::new();
        data.insert("hp_current".to_string(), json!(50.0));
        data.insert("hp_max".to_string(), json!(100.0));
        data.insert("in_combat".to_string(), json!(true));
        actor.set_data(data);
        
        // Start HP regeneration
        manager.start_regeneration(&actor, "hp_current").await.unwrap();
        
        // Check that regeneration should not continue (in combat)
        let should_continue = manager.should_continue_regeneration(&actor, "hp_current").await.unwrap();
        assert!(!should_continue);
    }
    
    #[tokio::test]
    async fn test_regeneration_calculation() {
        let config = RegenerationConfig::default();
        let manager = ResourceRegenerationManager::new(config);
        ...
        // Calculate regeneration amount for 10 seconds
        let regen_amount = manager.calculate_regeneration_amount(&actor, "hp_current", 10.0).await.unwrap();
        
        // Base rate is 0.1 per second, with vitality modifier
        // Expected: 0.1 * 10 * (1 + 15 * 0.1) = 1.0 * 2.5 = 2.5
        assert!(regen_amount > 0.0);
    }
}
