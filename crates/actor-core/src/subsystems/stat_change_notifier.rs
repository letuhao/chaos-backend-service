...
mod tests {
    ...
    #[tokio::test]
    async fn test_stat_change_notifier() {
        let config = NotifierConfig::default();
        let notifier = StatChangeNotifier::new(config);
        
        let actor = Actor::new("TestActor".to_string(), "Human".to_string());
        
        // Test stat change notification
        let changed_stats = vec!["lifespan".to_string(), "age".to_string()];
        notifier.notify_stat_change(&actor, &changed_stats).await.unwrap();
        
        // Check history
        let history = notifier.get_history(Some(&actor.id.to_string()), Some(10)).await.unwrap();
        assert_eq!(history.len(), 1);
        assert_eq!(history[0].changed_stats, changed_stats);
    }
    ...
    async fn test_resource_dependencies() {
        ...
        // Add resource dependency
        notifier.add_resource_dependency("hp_current".to_string(), "lifespan".to_string()).await.unwrap();
        notifier.add_resource_dependency("hp_current".to_string(), "vitality".to_string()).await.unwrap();
        
        // Test affected resources
        let changed_stats = vec!["lifespan".to_string()];
        let affected = notifier.get_affected_resources(&changed_stats).await.unwrap();
        assert!(affected.contains(&"hp_current".to_string()));
    }
    
    #[tokio::test]
    async fn test_needs_recalculation() {
        let config = NotifierConfig::default();
        let notifier = StatChangeNotifier::new(config);
        ...
        // Initially should not need recalculation
        let needs_recalc = notifier.needs_recalculation(&actor, "hp_current").await.unwrap();
        assert!(!needs_recalc);
        
        // After stat change, should need recalculation
        notifier.notify_stat_change(&actor, &["lifespan".to_string()]).await.unwrap();
        let needs_recalc = notifier.needs_recalculation(&actor, "hp_current").await.unwrap();
        assert!(needs_recalc);
    }
}
