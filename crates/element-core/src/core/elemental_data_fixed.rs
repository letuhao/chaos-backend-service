...
impl ElementalSystemData {
    ...
    pub fn set_element_mastery_level(&mut self, index: usize, level: f64) -> Result<(), String> {
        if index < MAX_ELEMENTS {
            self.element_mastery_levels[index] = level;
            Ok(())
        } else {
            Err(format!("Element index {} out of bounds (max: {})", index, MAX_ELEMENTS))
        }
    }
    
    /// Set element qi amount by index (direct array access - 1-2 ns)
    pub fn set_element_qi_amount(&mut self, index: usize, amount: f64) -> Result<(), String> {
        if index < MAX_ELEMENTS {
    ...
    pub fn set_element_power_point(&mut self, index: usize, power: f64) -> Result<(), String> {
        if index < MAX_ELEMENTS {
    ...
    pub fn set_element_defense_point(&mut self, index: usize, defense: f64) -> Result<(), String> {
        if index < MAX_ELEMENTS {
    ...
    pub fn calculate_derived_stats(&mut self, index: usize, base_damage: f64, base_defense: f64, base_crit_rate: f64, base_crit_damage: f64, base_accuracy: f64) -> Result<(), String> {
        if index >= MAX_ELEMENTS {
            return Err(format!("Element index {} out of bounds (max: {})", index, MAX_ELEMENTS));
        }
    ...
    /// Get total elemental mastery across all elements
    pub fn get_total_elemental_mastery(&self) -> f64 {
        self.element_mastery.iter().sum()
    }
